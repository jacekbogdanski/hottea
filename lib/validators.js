var { isUndefined, mapObj, curry, pipe } = require('./utils')
var { getChange, getData, putError, putChange } = require('./changeset')
...
module.exports = mapObj(
  {
    required({ fields, message = "can't be blank" }, changeset) {
      ...
      return fields.reduce((changeset, field) => {
        ...
        var data = getData(field, changeset)

        value = isUndefined(value) ? data : value
        return valid(value)
          ? changeset
          : pipe(putError(field, message), putChange(field, null))(changeset)
      }, changeset)
    },
    length({ message, min, max, fields }, changeset) {
      return fields.reduce((changeset, field) => {
        ...
        if (min == max && min == length)
          return putError(field, message.is(min), changeset)

        if (length < min) return putError(field, message.min(min), changeset)

        if (length > max) return putError(field, message.max(max), changeset)
