...
module.exports = mapObj(
  {
    ...
    inclusion(params, include, changeset) {
      return params.reduce((changeset, param) => {
        var value = getChange(param, changeset)
        if (isUndefined(value)) return changeset
        if (include.some(x => x === value)) return changeset

        return putError(param, 'is invalid', changeset)
      }, changeset)
    },
    format(params, regx, changeset) {
      return params.reduce((changeset, param) => {
        var value = getChange(param, changeset)
        if (isUndefined(value)) return changeset

        if (typeof value !== 'string')
          throw `Invalid type of ${param}. Valid types: string.`

        if (value.match(regx)) return changeset
        return putError(param, 'has invalid format', changeset)
      }, changeset)
    },
    lessThan(number, params, changeset) {
      return params.reduce((changeset, param) => {
        var value = getChange(param, changeset)
        if (isUndefined(value)) return changeset
        ...
        return value < number
          ? changeset
          : putError(param, `must be less than ${number}`, changeset)
      }, changeset)
    },
    greaterThan(number, params, changeset) {
      return params.reduce((changeset, param) => {
        var value = getChange(param, changeset)
        if (isUndefined(value)) return changeset
        ...
        return value > number
          ? changeset
          : putError(param, `must be greater than ${number}`, changeset)
      }, changeset)
    },
    lessThanOrEqualTo(number, params, changeset) {
      return params.reduce((changeset, param) => {
        var value = getChange(param, changeset)
        if (isUndefined(value)) return changeset
    ...
    greaterThanOrEqualTo(number, params, changeset) {
      return params.reduce((changeset, param) => {
        var value = getChange(param, changeset)
        if (isUndefined(value)) return changeset
    ...
    equalTo(number, params, changeset) {
      return params.reduce((changeset, param) => {
        var value = getChange(param, changeset)
        if (isUndefined(value)) return changeset
        ...
        return value == number
          ? changeset
          : putError(param, `must be equal to ${number}`, changeset)
      }, changeset)
    }
  },
  (key, fn) => curry(fn)
